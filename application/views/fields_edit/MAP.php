    ...
    <script>
        jQuery(document).ready(function($) {    
            ...
            map = L.map('<?php echo esc_attr($field_id); ?>_map', {
                center: [<?php echo esc_html(wmvc_show_data($field_id.'_lat', $db_data, get_option('wdk_default_lat', 51.505))); ?>, <?php echo esc_html(wmvc_show_data($field_id.'_lng', $db_data, get_option('wdk_default_lng', -0.09))); ?>],
                zoom: 4,
            });     
            ...
            marker = L.marker(
                [<?php echo esc_html(wmvc_show_data($field_id.'_lat', $db_data, get_option('wdk_default_lat', 51.505))); ?>, <?php echo esc_html(wmvc_show_data($field_id.'_lng', $db_data, get_option('wdk_default_lng', -0.09))); ?>],
                {draggable: true}
            ).addTo(map);
